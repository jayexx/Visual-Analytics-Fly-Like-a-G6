...
StudentLM_data <- merged_data_lm %>%
  ...
  summarize(
    ...
    # Compute Gini index for title_ID (question_gini)
    question_gini = Gini(table(title_ID)),

    total_diff_knowledge_areas = sum(colSums(across(29:36, as.numeric)) > 0),
    total_diff_sub_knowledge_areas = sum(colSums(across(14:28, as.numeric)) > 0),
    mean_question_scores = mean(question_score, na.rm = TRUE),
#    mean_memory_size = mean(memory, na.rm = TRUE),
    mean_memory_size_by_qns = mean(sapply(split(memory, title_ID), mean, na.rm = TRUE), na.rm = TRUE),
...
#    total_diff_answering_methods = n_distinct(method, na.rm = TRUE),
    mean_diff_answering_methods_by_qns = mean(sapply(split(method, title_ID), function(x) n_distinct(x, na.rm = TRUE)), na.rm = TRUE),

    # Compute Gini index for method (method_gini)
    method_gini = Gini(table(method)),
...
StudentLM_data <- StudentLM_data %>%
  select(-total_diff_knowledge_areas, 
#         -total_timeconsume, 
#         -total_memory_size, 
#         -total_answering_attempts, 
#         -mean_diff_answering_methods_by_qns, 
#         -question_gini, 
#         -total_diff_questions_attempted, 
         -total_diff_sub_knowledge_areas)
...
-   Proportion of absolutely and partially correct attempts: - absolutely correct attempts - award 1 pt - partially correct attempts - award (actual_score / question_score) - wrong attempts - penalise 1 pt - normalise attempts across questions - uses (total point / total attempts)\
...
StudentKA_data <- merged_data %>%
  ...
  summarize(
    ...
    sum_highest_actual_score_b3C9s = sum(sapply(unique(title_ID), function(x) {
      ifelse(any(merged_data[merged_data$title_ID == x & merged_data$student_ID == cur_group()$student_ID, 29] == 1),
             max(merged_data[merged_data$title_ID == x & merged_data$student_ID == cur_group()$student_ID & merged_data[, 29] == 1, "actual_score"]),
             0)
    })),
    sum_highest_actual_score_g7R2j = sum(sapply(unique(title_ID), function(x) {
      ifelse(any(merged_data[merged_data$title_ID == x & merged_data$student_ID == cur_group()$student_ID, 30] == 1),
             max(merged_data[merged_data$title_ID == x & merged_data$student_ID == cur_group()$student_ID & merged_data[, 30] == 1, "actual_score"]),
             0)
    })),
    sum_highest_actual_score_k4W1c = sum(sapply(unique(title_ID), function(x) {
      ifelse(any(merged_data[merged_data$title_ID == x & merged_data$student_ID == cur_group()$student_ID, 31] == 1),
             max(merged_data[merged_data$title_ID == x & merged_data$student_ID == cur_group()$student_ID & merged_data[, 31] == 1, "actual_score"]),
             0)
    })),
    sum_highest_actual_score_m3D1v = sum(sapply(unique(title_ID), function(x) {
      ifelse(any(merged_data[merged_data$title_ID == x & merged_data$student_ID == cur_group()$student_ID, 32] == 1),
             max(merged_data[merged_data$title_ID == x & merged_data$student_ID == cur_group()$student_ID & merged_data[, 32] == 1, "actual_score"]),
             0)
    })),
    sum_highest_actual_score_r8S3g = sum(sapply(unique(title_ID), function(x) {
      ifelse(any(merged_data[merged_data$title_ID == x & merged_data$student_ID == cur_group()$student_ID, 33] == 1),
             max(merged_data[merged_data$title_ID == x & merged_data$student_ID == cur_group()$student_ID & merged_data[, 33] == 1, "actual_score"]),
             0)
    })),
    sum_highest_actual_score_s8Y2f = sum(sapply(unique(title_ID), function(x) {
      ifelse(any(merged_data[merged_data$title_ID == x & merged_data$student_ID == cur_group()$student_ID, 34] == 1),
             max(merged_data[merged_data$title_ID == x & merged_data$student_ID == cur_group()$student_ID & merged_data[, 34] == 1, "actual_score"]),
             0)
    })),
    sum_highest_actual_score_t5V9e = sum(sapply(unique(title_ID), function(x) {
      ifelse(any(merged_data[merged_data$title_ID == x & merged_data$student_ID == cur_group()$student_ID, 35] == 1),
             max(merged_data[merged_data$title_ID == x & merged_data$student_ID == cur_group()$student_ID & merged_data[, 35] == 1, "actual_score"]),
             0)
    })),
    sum_highest_actual_score_y9W5d = sum(sapply(unique(title_ID), function(x) {
      ifelse(any(merged_data[merged_data$title_ID == x & merged_data$student_ID == cur_group()$student_ID, 36] == 1),
             max(merged_data[merged_data$title_ID == x & merged_data$student_ID == cur_group()$student_ID & merged_data[, 36] == 1, "actual_score"]),
             0)
    })),
...
mastery_scores <- mastery_scores2 %>%
  ...
  summarize(
    ...
    sum_points_overall = sum(adjusted_points),
    
    # Part (b): Sum of highest actual score for each knowledge area
    sum_points_b3C9s = sum(case_when(
      b3C9s == 1 ~ adjusted_points,
      TRUE ~ 0
    )),
    
    sum_points_g7R2j = sum(case_when(
      g7R2j == 1 ~ adjusted_points,
      TRUE ~ 0
    )),
    
    sum_points_k4W1c = sum(case_when(
      k4W1c == 1 ~ adjusted_points,
      TRUE ~ 0
    )),
    
    sum_points_m3D1v = sum(case_when(
      m3D1v == 1 ~ adjusted_points,
      TRUE ~ 0
    )),
    
    sum_points_r8S3g = sum(case_when(
      r8S3g == 1 ~ adjusted_points,
      TRUE ~ 0
    )),
    
    sum_points_s8Y2f = sum(case_when(
      s8Y2f == 1 ~ adjusted_points,
      TRUE ~ 0
    )),
    
    sum_points_t5V9e = sum(case_when(
      t5V9e == 1 ~ adjusted_points,
      TRUE ~ 0
    )),
    
    sum_points_y9W5d = sum(case_when(
...
g <- ggplot(StudentLMKA_data, 
       aes(x = sum_highest_actual_score_k4W1c, 
           y = cluster,
           fill = factor(stat(quantile))
           )) +
  stat_density_ridges(
...
h <- ggplot(StudentLMKA_data, 
       aes(x = sum_highest_actual_score_s8Y2f, 
           y = cluster,
           fill = factor(stat(quantile))
           )) +
  stat_density_ridges(
...
pct_answers_weekdays pct_answers_working_hours total_diff_questions_attempted question_gini mean_question_scores mean_memory_size_by_qns mean_timeconsume total_answering_attempts mean_diff_answering_methods_by_qns method_gini total_memory_size total_timeconsume cluster sum_highest_actual_score sum_highest_actual_score_b3C9s sum_highest_actual_score_g7R2j sum_highest_actual_score_k4W1c sum_highest_actual_score_m3D1v sum_highest_actual_score_r8S3g sum_highest_actual_score_s8Y2f sum_highest_actual_score_t5V9e sum_highest_actual_score_y9W5d pct_answers_absolutely_correct total_questions_correct_or_partial sum_points_overall sum_points_b3C9s sum_points_g7R2j sum_points_k4W1c sum_points_m3D1v sum_points_r8S3g sum_points_s8Y2f sum_points_t5V9e sum_points_y9W5d

```{r}
# Multi linear regression model for sum_highest_actual_score and sum_points overall
model1 <- lm(sum_highest_actual_score ~ 
               pct_answers_weekdays +
               pct_answers_working_hours+
               total_diff_questions_attempted+
               question_gini+
               mean_question_scores+
               mean_memory_size_by_qns+
               mean_timeconsume+
               total_answering_attempts+
               mean_diff_answering_methods_by_qns+
               method_gini+
               total_memory_size+
               total_timeconsume, data = StudentLMKA_data)
model2 <- lm(sum_points_overall ~ 
               pct_answers_weekdays +
               pct_answers_working_hours+
               total_diff_questions_attempted+
               question_gini+
...
#| fig-width: 18
#| fig-height: 16
a <- ggcoefstats(model1, 
            output = "plot")
b <- ggcoefstats(model2, 
            output = "plot")
