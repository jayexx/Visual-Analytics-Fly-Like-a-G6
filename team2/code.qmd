...
clustering_data <- learning_behavior %>%
  select(avg_rate, avg_time_consume, b3C9s, g7R2j, k4W1c, m3D1v, r8S3g, s8Y2f, t5V9e, y9W5d)
...
cluster_means <- learning_behavior %>%
  group_by(cluster) %>%
  summarise(across(c(avg_rate, avg_time_consume, b3C9s, g7R2j, k4W1c, m3D1v, r8S3g, s8Y2f, t5V9e, y9W5d), mean, na.rm = TRUE))
...
# 绘制每个聚类在各个特征上的差异
ggplot(cluster_means_long, aes(x = feature, y = value, fill = factor(cluster))) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Feature Differences Across Clusters",
       x = "Feature",
       y = "Mean Value",
       fill = "Cluster") +
