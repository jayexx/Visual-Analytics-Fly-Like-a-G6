...
# Variables to plot
variables <- names(StudentLM_data)[2:11]
...
SLM.cor <- cor(StudentLM_data[, 2:11])
...
Inspecting the data frame, 2 variables were found to be highly skewed and concentrated within a small range of values, hence they are removed for more meaningful analysis, with the following code chunk. For high correlation with a threshold of >0.8, the removed variables are highly correlated to total different questions attempted, which is preserved in the data frame.

```{r}
StudentLM_data <- StudentLM_data %>%
  select(-total_diff_knowledge_areas, -total_diff_sub_knowledge_areas)
...
An Alluvial plot is also used in the following code chunk for an alterantive visualisation of the clustering, where variables are binned into 5 bin equally sized bins.

```{r}
# Define a function to bin numerical variables
bin_variable <- function(x, bins = 3, labels = NULL) {
  cut(x, breaks = bins, labels = labels)
}

# Apply binning to numerical columns
StudentLM_data_binned <- StudentLM_data %>%
  mutate(across(-student_ID, ~ bin_variable(., bins = 3)))
...
ggplot(StudentLM_data_long,
       aes(x = variable, stratum = value, alluvium = student_ID, fill = cluster, label = value)) +
  geom_flow(stat = "alluvium", lode.guidance = "forward", color = "darkgray") +
  geom_stratum() +
  geom_text(stat = "stratum", aes(label = after_stat(stratum))) +
  scale_x_discrete(limits = unique(StudentLM_data_long$variable), expand = c(0.15, 0.05)) +
  theme_minimal() +
  labs(title = "Alluvial Plot of Clusters Dividing Among Variables",
       x = "Variables",
       y = "Count") +
...
StudentLM_data1 <- select(StudentLM_data, c(1, 2:11))
